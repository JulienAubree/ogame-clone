...
interface HomepageImageSlotProps {
  /** Logical slot key — used as both the upload target and the file path. */
  slot: string;
  /** Current path (typically `/assets/landing/<slot>.webp`). */
  value: string;
  /** Suggested aspect ratio for preview. */
  aspect?: string;
  label: string;
  hint?: string;
...
/**
 * Upload widget for a landing-page image slot. Uploads via the existing
 * `/admin/upload-asset` route with category=landing, then reports the public
 * path back via `onChange` so the parent can save it on the homepage blob.
 */
export function HomepageImageSlot({
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `/assets/landing/${slot}.webp`;
      onChange(path);
