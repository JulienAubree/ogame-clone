...
// ── Active Ability Card (extracted for hooks compliance) ──

function ActiveAbilityCard({ ability, cooldownData, styles, isActive, scanTarget, setScanTarget, handleScan, scanMutation, scanError }: {
  ability: any;
  cooldownData: { cooldownEnds: string } | undefined;
  styles: { badge: string; badgeText: string };
  isActive: boolean;
  scanTarget: { galaxy: number; system: number; position: number };
  setScanTarget: (coords: { galaxy: number; system: number; position: number }) => void;
  handleScan: () => void;
  ...
  const secondsLeft = useCountdown(onCooldown ? cooldownEnd : null);
