...
- **Pas de nouvelle table pour les avatars** : fichiers statiques dans `public/assets/avatars/`, geres via l'admin panel
...
|---------|------|---------|-------------|
| `bio` | text, nullable | `null` | Description libre, max 500 caracteres |
| `avatarId` | text, nullable | `null` | Nom du fichier avatar (ex: `commander-01`) |
| `playstyle` | text, nullable | `null` | `'miner'`, `'warrior'`, `'explorer'` ou `null` |
| `seekingAlliance` | boolean | `false` | Indique si le joueur cherche une alliance |
| `theme` | text | `'dark'` | `'dark'` ou `'light'` |
| `profileVisibility` | jsonb | `{"bio":true,"playstyle":true,"stats":true}` | Controle ce que les autres joueurs voient |
...
- `requesterId != addresseeId` (pas d'auto-amitie)

### Galerie d'avatars

Les avatars sont des fichiers statiques (portraits de commandants pixel art) dans `apps/web/public/assets/avatars/`. L'admin uploade les images via le panel existant. Le champ `avatarId` sur `users` stocke le nom de fichier sans extension (ex: `commander-01`). Le frontend construit l'URL : `/assets/avatars/{avatarId}.webp`.

L'endpoint `user.listAvatars` scanne le dossier pour retourner la liste des avatars disponibles.
...
| `user.getProfile` | query | `{ userId: string }` | Profil public d'un joueur. Filtre les champs selon `profileVisibility`. Inclut le statut de la relation d'amitie avec le joueur courant (`none`, `pending_sent`, `pending_received`, `friends`). |
| `user.getMyProfile` | query | — | Mon profil complet avec toutes les donnees, sans filtre de visibilite |
| `user.updateProfile` | mutation | `{ bio?, avatarId?, playstyle?, seekingAlliance?, theme?, profileVisibility? }` | Met a jour mon profil. Valide : bio max 500 chars, avatarId doit exister dans la galerie, playstyle parmi les valeurs autorisees. |
| `user.listAvatars` | query | — | Liste les noms de fichiers disponibles dans `assets/avatars/` |

### Nouveau router `friend`

| Procedure | Type | Input | Description |
|-----------|------|-------|-------------|
| `friend.list` | query | — | Liste mes amis acceptes (id, username, avatarId) |
...
| `friend.decline` | mutation | `{ friendshipId: string }` | Refuser une demande recue. |
| `friend.remove` | mutation | `{ friendshipId: string }` | Supprimer un ami (fonctionne pour les deux cotes de la relation). |
...
- Demande envoyee → "Demande envoyee" (desactive)
...
- **`FriendRequests`** — demandes recues et envoyees avec boutons d'action (accepter/refuser).

### Points d'entree dans la navigation

- Lien "Profil" dans la sidebar et le bottom tab bar (icone existante `social`)
